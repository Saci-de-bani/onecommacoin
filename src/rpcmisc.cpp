...
Value getinfo(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            ...
            "  \"balance\": xxxxxxx,         (numeric) the total onecommacoin balance of the wallet\n"
            "  \"Darksend_balance\": xxxxxx, (numeric) the anonymized onecommacoin balance of the wallet\n"
            ...
            "  \"paytxfee\": x.xxxx,         (numeric) the transaction fee set in onecommacoin/kb\n"
            "  \"relayfee\": x.xxxx,         (numeric) minimum relay fee for non-free transactions in onecommacoin/kb\n"
...
Value validateaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "validateaddress \"onecommacoinaddress\"\n"
            "\nReturn information about the given onecommacoin address.\n"
            "\nArguments:\n"
            "1. \"onecommacoinaddress\"     (string, required) The onecommacoin address to validate\n"
            "\nResult:\n"
            "{\n"
            "  \"isvalid\" : true|false,         (boolean) If the address is valid or not. If not, this is the only property returned.\n"
            "  \"address\" : \"onecommacoinaddress\", (string) The onecommacoin address validated\n"
...
#ifdef ENABLE_WALLET
        // Case 1: OCC address and we have full public key:
...
Value createmultisig(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 2) {
        string msg = "createmultisig nrequired [\"key\",...]\n"
                     ...
                     "2. \"keys\"       (string, required) A json array of keys which are onecommacoin addresses or hex-encoded public keys\n"
                     "     [\n"
                     "       \"key\"    (string) onecommacoin address or hex-encoded public key\n"
                     "       ,...\n"
                     "     ]\n"
...
Value verifymessage(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 3)
        throw runtime_error(
            "verifymessage \"onecommacoinaddress\" \"signature\" \"message\"\n"
            "\nVerify a signed message\n"
            "\nArguments:\n"
            "1. \"onecommacoinaddress\"  (string, required) The onecommacoin address to use for the signature.\n"
