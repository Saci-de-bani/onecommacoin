...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("OCCMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("OCCMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("OCCMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("onecommacoin-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        if (fProofOfStake) {
            ...
            if (!pblock->SignBlock(*pwallet)) {
                LogPrintf("OCCMiner(): Signing new block failed \n");
                continue;
            }
        ...
        LogPrintf("Running OCCMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void static ThreadBitcoinMiner(void* parg)
{
    ...
    try {
        ...
        LogPrintf("ThreadOCCMiner() exception");
    } catch (...) {
        LogPrintf("ThreadOCCMiner() exception");
    }

    LogPrintf("ThreadOCCMiner exiting\n");
}
