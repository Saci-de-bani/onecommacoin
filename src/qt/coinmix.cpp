...
class TxViewDelegate2 : public QAbstractItemDelegate
{
    Q_OBJECT
public:
    TxViewDelegate2() : QAbstractItemDelegate(), unit(BitcoinUnits::OCC)
    {
    }
...
void Coinmix::updateDarksendProgress()
{
    ...
    QString strAnonymizeOCCAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeOCCAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if (currentBalance == 0) {
        ...
        strAnonymizeOCCAmount = strAnonymizeOCCAmount.remove(strAnonymizeOCCAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeOCCAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if (nMaxToAnonymize > nAnonymizeOCCAmount * COIN) nMaxToAnonymize = nAnonymizeOCCAmount * COIN;

    if (nMaxToAnonymize == 0) return;

    if (nMaxToAnonymize >= nAnonymizeOCCAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                              .arg(strAnonymizeOCCAmount));
        strAnonymizeOCCAmount = strAnonymizeOCCAmount.remove(strAnonymizeOCCAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeOCCAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
