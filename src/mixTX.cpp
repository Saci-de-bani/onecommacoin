        ...
        //check it like a transaction
        {
            ...
            BOOST_FOREACH (const CTxOut o, out) {
                ...
                if (o.scriptPubKey.size() != 25) {
                    ...
                    errorID = ERR_NON_SOCCDARD_PUBKEY;
                    pfrom->PushMessage("dssu", sessionID, GetState(), GetEntriesCount(), MASTERNODE_REJECTED, errorID);
                    return;
                }
    ...
    if (fMasterNode) {
        ...
        BOOST_FOREACH (const CTransaction& txCollateral, vecSessionCollateral) {
            ...
            /*
                ...
                allow endless transaction that would bloat OCC and make it unusable. To
                stop these kinds of attacks 1 in 10 successful transactions are charged. This
                adds up to a cost of 0.001 OCC per transaction on average.
            */
            if (r <= 10) {
...
bool CDarksendPool::DoAutomaticDenominating(bool fDryRun)
{
    ...
    CAmount nBalanceNeedsAnonymized = nAnonymizeOCCAmount * COIN - pwalletMain->GetAnonymizedBalance();
...
void CDarksendPool::GetDenominationsToString(int nDenom, std::string& strDenom)
{
    ...
    //
    // bit 0 - 100OCC+1 ( bit on if present )
    // bit 1 - 10OCC+1
    // bit 2 - 1OCC+1
    // bit 3 - .1OCC+1
    // bit 3 - non-denom
...
void ThreadCheckDarKsendPool()
{
    ...
    RenameThread("onecommacoin-Darksend");
